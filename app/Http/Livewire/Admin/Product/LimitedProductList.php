...
class LimitedProductList extends Component
{
    ...
    {
        $productQuery = Product::withSum('productQuantities', 'quantity')->withSum('productOrders', 'qty')->withSum('returnProductsQuantity', 'qty')->withSum('productReductions', 'qty')->where('store_id', $this->storeUser)->whereColumn('quantity', '<=' ,'default_quantity')->where('quantity', '>', 0);
