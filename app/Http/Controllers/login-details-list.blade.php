                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if(count($data->shift_details)>0)
                                    @foreach($data->shift_details as $details)
                                        {{$details->shift==1?'Morning':'Evening'}}<br>
                                    @endforeach
                                    @endif
                                </td>
                                <td>
                                    @if(count($data->shift_details)>0)
                                    @foreach($data->shift_details as $details)
                                        {{date('h:i A',strtotime($details->login_time)) }}<br>
                                    @endforeach
                                    @endif
                                ...
                                <td>
                                    @if(count($data->shift_details)>0)
                                    @foreach($data->shift_details as $details)
                                        @if($details->logout_time){{date('h:i A',strtotime($details->logout_time)) }}<br>@endif
                                    @endforeach
                                    @endif</td>
