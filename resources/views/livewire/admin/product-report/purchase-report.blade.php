			...
			<div class="card-body">
				...
				@php
                    function convert_numbers_to_indian_format($number)
                    {
                        ...
                        if ($decimal != '0') {
							$checkDecimal = explode('.',$decimal);
							if(strlen($checkDecimal[1]) == 1){

								$formattedAmount = $formattedAmount . $decimal.'0';
								}
								else{
				...
				<div class="row mb-3">
					<div class="col-lg-3">
						...
						<input type="text" wire:model.defer="note" class="form-control" placeholder="Description">
						@error('note') <span class="text-danger error">{{ $message }}</span>@enderror
					</div>

					<div class="col-lg-3">
						<label class="form-label">Date</label>
						<input class="form-control" wire:model.defer="date" type="date" name="date" placeholder="Date From">
						@error('date') <span class="text-danger error">{{ $message }}</span>@enderror
					</div>

					<div class="col-lg-3">
						<label class="form-label">Price</label>
						<input type="text" wire:model.defer="purchase_price" class="form-control" placeholder="Price"  onkeypress="return decimal_number_check(event);">
				...
				<div class="row mb-2">
					<div class="col-xl-9">
						<div class="row gy-2 gx-2 align-items-center justify-content-xl-start justify-content-between">
							...
							<div class="col-auto">
								<div class="mb-3">
									<input class="form-control" type="date" wire:model.lazy="dateTo" placeholder="Date To">
								</div>
							</div>
							<div class="col-auto">
								<div class="mb-3">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-centered w-100 dt-responsive nowrap">
						...
						<tbody>
							@php
								$total =$reports->sum('purchase_price');
								$formatter = new NumberFormatter("en_IN", NumberFormatter::CURRENCY);
								//dd($formatter->formatCurrency($total, "INR"))
							@endphp
							@if(count($reports)>0)
							...
							@php
								...
								<td>{{env('CURRENCY','₹')}}{{$formatter->formatCurrency($row->purchase_price, "INR")}}</td>
								 @if (Auth::user()->type == 'A')
							...
							<td colspan="1">
								{{ env('CURRENCY', '₹') }}{{ $formatter->formatCurrency($total, "INR") }}
							</td>
                            @else
                            <tr>
								 @if (Auth::user()->type == 'A')
						...
						<table class="table w-auto table-dark">
							<tbody>
								...
								<tr>
									...
									<th>{{ env('CURRENCY', '₹') }}{{ $formatter->formatCurrency($total_purchase_price, "INR") }}</th>
								</tr>                                                
							</tbody>
						</table>
            ...
            <div class="modal-body">
                <div class="row mt-2 mb-2">
                    ...
                    <div class="mb-3 col-md-4">
                        ...
                        <input type="text" wire:model.defer="edit_note" class="form-control" placeholder="Note">
                        @error('edit_note') <span class="text-danger error">{{ $message }}</span>@enderror
                    </div>

                    <div class="mb-3 col-md-4">
                        <label class="form-label">Date</label>
                        <input type="date" wire:model.defer="edit_date" class="form-control" placeholder="Enter Name">
                        @error('edit_date') <span class="text-danger error">{{ $message }}</span>@enderror
                    </div>

                    <div class="mb-3 col-md-4">
                        <label class="form-label">Purchase Price</label>
                        <input type="text" wire:model.defer="edit_purchase_price" class="form-control" placeholder="Purchase Price"  onkeypress="return decimal_number_check(event);">
                        @error('edit_purchase_price') <span class="text-danger error">{{ $message }}</span>@enderror
                    </div>
